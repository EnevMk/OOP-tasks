...
Definition Dictionary::getMeaning(const String word_) const {
    ...
    for (int i = 0; i < v.size(); ++i) {
        ...
        if (v[i].word == word_) {
            //return std::cout << v[i].word << v[i].definition;
...
Dictionary Dictionary::operator+(Dictionary &obj) const {
    ...
    /* for (int i = 0; i < res.v.size(); ++i) {
        ...
        for (int j = 0; j < obj.v.size(); ++j) {
            ...
            if (res.v[i].word == obj.v[j].word) {
                res.v[i].definition = res.v[i].definition + obj.v[j].definition;
                obj.v.removeAtIndex(j);
            }
    ...
    for (int i = 0; i < endInd - 1; ++i) {
        ...
        for (int j = 1; j < endInd; ++j) {
            ...
            if (res.v[i].word == res.v[j].word && i != j) {
                ...
                std::cout << "in operator + : " << res.v[j].definition << '\n';
